...
fn decode_report(raw_report: &[u8]) -> Result<Report, ContractError> {
    ...
    let (len, raw_report) = raw_report
        .split_first()
        .ok_or(ContractError::InvalidRawReport)?;
    ...
    let juels_per_fee_coin = u128::from_be_bytes(
        raw_report
            .try_into()
            .map_err(|_| ContractError::InvalidJeuls)?,
    );
...
fn report(
    ...
    require!(report.observations.len() <= MAX_ORACLES, InvalidObservationLengthMoreThanOracles);
    require!(
        usize::from(config.f) < report.observations.len(),
        InvalidObservationLength
    );
